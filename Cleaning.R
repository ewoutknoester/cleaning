...
#per groep
aggregate(cbind(P.value=Survival_kopie$survival) ~ Survival_kopie$size + Survival_kopie$species + Survival_kopie$date + Survival_kopie$cleaning, data = Survival_kopie, function(x) shapiro.test(Survival_kopie$survival)$p.value
aggregate(cbind(P.value=Production$SGR) ~ Production$size + Production$species + Production$date + Production$cleaning, data = Production, function(x) shapiro.test(Production$SGR)$p.value
aggregate(cbind(P.value=Growth$SGR) ~ Growth$size + Growth$species + Growth$date + Growth$cleaning, data = Growth, function(x) shapiro.test(Growth$SGR)$p.value
